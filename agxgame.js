...
function hostPrepareGame(data) {
  ...
  const domain =
    NODE_ENV === 'prod'
      ? 'https://fibbage-tribute-questions.herokuapp.com'
      : 'http://localhost:3000';
...
function allPloysSent(data) {
  const ploys = shuffle(data.ploys.slice());

  // Pick a random spot in the decoy list to put the correct answer
  const rnd = Math.floor(Math.random() * (ploys.length + 1));
  ploys.splice(rnd, 0, {
    playerId: 'answer',
    value: data.answer,
  });
...
function sendQuestion(wordPoolIndex, gameId) {
  /*
    ...
    */
  const json = questions[wordPoolIndex];
  json.answer = json.solution;
  json.round = wordPoolIndex;
  io.sockets.in(gameId).emit('newQuestion', json);
}
